...
const submit = () => {
  confirm('この内容で更新しますか？') ? router.post(route('owner.product.update'), form) : '';
}

</script>

<template>
  <Head title="管理画面 商品 編集" />

  <OwnerAuthenticatedLayout>
    <template #header>
      <h2 class="font-semibold text-xl text-gray-800 leading-tight">商品 編集</h2>
    </template>
                ...
                <div class="container px-5 py-10 mx-auto">
                  <form @submit.prevent="submit" class="md:w-2/3 mx-auto">
                    <div class="p-2 w-full">
                      ...
                      <select id="manufacturer_id" name="manufacturer_id" v-model="form.manufacturer_id"
                    ...
                    <div class="p-2 w-full">
                      ...
                      <input type="text" v-model="form.product_name" id="product_name" name="product_name"
                        class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                    </div>
                    <div class="p-2 w-full">
                    ...
                    <div class="p-2 w-full">
                      <div>
                        <div for="picture" class="leading-7 text-sm text-gray-600">画像添付:{{ product.picture ? '有' : '無' }}
                        </div>
                    ...
                    <Link as="button" :href="route('owner.product.show', { id: product.id })"
